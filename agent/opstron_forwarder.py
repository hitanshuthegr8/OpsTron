...
USAGE:
  ...
  export OPSTRON_BACKEND_URL="https://OpsTronicic.onrender.com"
  python3 opstronic_forwarder.py

ENVIRONMENT VARIABLES:
  OPSTRON_API_KEY      (required) Your unique agent key from the OpsTronic dashboard
  OPSTRON_BACKEND_URL  (required) Base URL of the OpsTronic backend (no trailing slash)
  OPSTRON_POLL_SECS    (optional) How often to poll for new logs, default 30
  OPSTRON_LABEL        (optional) Label to filter on, default OpsTronic.monitor=true
...
BACKEND_URL  = os.environ.get("OPSTRON_BACKEND_URL", "https://OpsTronicic.onrender.com").rstrip("/")
POLL_SECS    = int(os.environ.get("OPSTRON_POLL_SECS", "30"))
MONITOR_LABEL = os.environ.get("OPSTRON_LABEL", "OpsTronic.monitor")
AGENT_VERSION = "4.0.0"
